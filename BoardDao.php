...
class BoardDao {
    ...
    // 새 글을 DB에 추가
    public function insertMsg($writer, $title, $content, $save_name) {
        try {
            $query = $this->db->prepare("insert into s_board
                         (writer, title, content, regtime, hits, save_name)
                  values (:writer, :title, :content, :regtime, 0, :save_name)");
			...
			$query->bindValue(":save_name", $save_name, PDO::PARAM_STR);
            $query->execute();
